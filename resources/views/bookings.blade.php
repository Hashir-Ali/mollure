        ...
        <tr class="bk_tr_{{$books->id}}  @if($books->status=='cancel') cancel_bk_tr @endif">
          ...
          <td class="text-left bdr_top bdr_left v-align-top" rowspan=" {{ $books->total_units + 1 }} ">
            @if($books->location_type=='f')
              FL: @if(isset($books->booking_professional[0]->prof_fix_location) && count($books->booking_professional[0]->prof_fix_location)>0)
              ...
              <td class="text-center bdr_left px-2 py-2 v-align-top">
                <div class="d-flex justify-content-center">
                  <div class="w-100">
                    ...
                    @if($book_trans->booking_service->service_name)
                    {{$book_trans->booking_service->service_name}}
                    @endif <br> {{$book_trans->duration}}   
                  </div>
                  <div>
